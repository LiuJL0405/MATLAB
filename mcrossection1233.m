...
line(0,-max(ppz):0.01:max(ppz),'linestyle','-','color','r','LineWidth',1)
    ...
    % ECE Diagnostic Calculation
    B=2.2486; % Magnetic field at the magnetic axis in Tesla
    frequencies=105.1:2:167.1; % Frequency range in GHz
    omega_c=1.76e11*B; % Cyclotron frequency in Hz for electrons
    R_ECE=(frequencies*1e9)*R0/omega_c; % Calculate the corresponding radius for each frequency
    
    % Plotting ECE diagnostic points
    plot(R_ECE, zeros(size(R_ECE)), 'ro'); 
    % Plot red circles at the calculated positions
