...
irun=0;
nt=500;%时间步数
runtimea=cputime;%记录当前 CPU 时间，用于后续计算代码运行时间
...
if(irun==0)
	...
	ky=data(id,1); % k_perp
	wr=real(data(id,2));
    wi=imag(data(id,2));%选择第4组数据(ky=1.0)作为初始参数

	runtime=cputime;

	tau=1.0;%T0e/T0i
	epsn=0.2;%L_n/R，归一化梯度参数
	kapn=1/epsn; % R/L_n，归一化密度梯度标长
	kapt=0.1*kapn; % R/L_T，归一化温度梯度标长
	eta=kapt/kapn;%梯度比
    ...
    %besseli(0,ki^2,1)计算的是exp(-ki^2)*besseli(0,ki^2)即0阶第一类修正贝塞尔函数在ki^2处的值乘以exp(-ki^2)
    Gamma0i=besseli(0,ki^2,1);
    Gamma0e=besseli(0,ke^2,1);
    G0coef=1.0/((1.0-Gamma0i)+(1.0-Gamma0e)/tau)/sqrt(2.0*pi);%电势前面的系数

    %设置速度空间网格点数和时间步长，定义速度范围(vx, vy)和网格间距
    ...
    wDiv=wdi*(vx.^2+vy.^2/2)+kzi*vx; % 离子磁漂频率+k||*v||
    wTiv=wdi*(kapn+(0.5*(vx.^2+vy.^2)-1.5)*kapt); % 离子温度梯度频率
    wDev=wde*(vx.^2+vy.^2/2)+kze*vx; % 电子磁漂频率+k||*v||
    wTev=wde*(kapn+(0.5*(vx.^2+vy.^2)-1.5)*kapt); % 电子温度梯度频率
    
    F0=exp(-0.5*(vx.^2+vy.^2)); % 初始Maxwell分布
...
%         xlabel('t'); legend('Re\phi','Im\phi'); legend('boxoff');
% %         xlim([0,it*dt]);
